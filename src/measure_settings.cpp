...
MeasureSettings::MeasureSettings(CapturePlot *plot, QWidget *parent)
	: QWidget(parent), m_ui(new Ui::MeasureSettings), m_channelName(""),
	  m_chnUnderlineColor(),
	  m_horizMeasurements(new DropdownSwitchList(2, this)),
	  m_vertMeasurements(new DropdownSwitchList(2, this)),
	  m_emitActivated(true), m_emitStatsChanged(true),
	  m_emitStatsDeleteAll(true), m_emitDeleteAll(true),
	  m_are_dropdowns_filled(false), m_plot(plot), m_selectedChannel(-1),
