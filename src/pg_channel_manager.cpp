...
//////////////////////// CHANNEL
bool PatternGeneratorChannel::getOutputMode() const { return outputMode; }

void PatternGeneratorChannel::setOutputMode(bool value) { outputMode = value; }
...
qreal PatternGeneratorChannel::getCh_thickness() const { return ch_thickness; }
...
PatternGeneratorChannelUI::PatternGeneratorChannelUI(
	PatternGeneratorChannel *ch, PatternGeneratorChannelGroup *chg,
	PatternGeneratorChannelGroupUI *chgui,
	PatternGeneratorChannelManagerUI *managerUi, QWidget *parent)
	: ChannelUI(ch, parent), managerUi(managerUi), chg(chg), chgui(chgui) {
	this->ch = ch;
...
bool PatternGeneratorChannelUI::eventFilter(QObject *watched, QEvent *event) {
	if (event->type() == QEvent::DragEnter) {
		...
		if (!enterEvent->mimeData()->hasFormat(
			    "patterngenerator/channelgroup") &&
		    !enterEvent->mimeData()->hasFormat(
			    "patterngenerator/channel")) {
			return true;
		}
	}
	...
	if (event->type() == QEvent::Drop) {
		...
		if (!dropEvent->mimeData()->hasFormat(
			    "patterngenerator/channelgroup") &&
		    !dropEvent->mimeData()->hasFormat(
			    "patterngenerator/channel")) {
			return true;
		}
	}
...
void PatternGeneratorChannelUI::dragEnterEvent(QDragEnterEvent *event) {
	...
	if (event->mimeData()->hasFormat("patterngenerator/channelgroup")) {
		short from = (short)event->mimeData()->data(
			"patterngenerator/channelgroup")[1];
		auto channelGroups = getManagerUi()->chm->get_channel_groups();
		auto chgIter = std::find(channelGroups->begin(),
					 channelGroups->end(), chg);
		auto chgIndex = chgIter - channelGroups->begin();
...
void PatternGeneratorChannelUI::dropEvent(QDropEvent *event) {
	...
	if (event->mimeData()->hasFormat("patterngenerator/channelgroup")) {
		short from = (short)event->mimeData()->data(
			"patterngenerator/channelgroup")[1];
		auto fromNrOfChannels = getManagerUi()
						->chm->get_channel_group(from)
						->get_channel_count();
		getManagerUi()->chm->join({(int)chgIndex, from});
		getManagerUi()->chm->highlightChannel(
	...
	if (event->mimeData()->hasFormat("patterngenerator/channel")) {
		short fromChg = (short)event->mimeData()->data(
			"patterngenerator/channel")[1];
		short fromCh = (short)event->mimeData()->data(
			"patterngenerator/channel")[3];

		auto fromNrOfChannels =
			getManagerUi()
				->chm->get_channel_group(fromChg)
				->get_channel_count();
		...
		auto chgIter = std::find(channelGroups->begin(),
					 channelGroups->end(), chg);
		// getManagerUi()->chm->move(fromChg, chgIndex +
		// ((fromChg)>chgIndex ? 0 : 1),dropAfter);
		getManagerUi()->chm->moveChannel(
			chgIndex, (*chgIter)->get_channel_count() - 1, chIndex,
			dropAfter);
	}
...
void PatternGeneratorChannelUI::updateTrace() {
	...
	if (ch->getCh_thickness() != trace->getCh_thickness()) {
		...
		getManagerUi()->main_win->view_->time_item_appearance_changed(
			false, true);
	}

	auto height = geometry().height();
	auto v_offset = topSep->geometry().bottomRight().y() + 3 + height -
		(trace->v_extents().second) + chgui->getTraceOffset() +
		3; // chgOffset.y();
	...
	getManagerUi()->main_win->view_->time_item_appearance_changed(true,
								      true);
}
...
PatternGeneratorChannelUI::getManagerUi() const {
	return managerUi;
}
...
PatternGeneratorChannelGroup::PatternGeneratorChannelGroup(
	PatternGeneratorChannel *ch, bool en)
	: ChannelGroup(ch) {
	collapsed = false;
	enabled = false;
...
PatternGeneratorChannel *PatternGeneratorChannelGroup::get_channel(int index) {
	return static_cast<PatternGeneratorChannel *>(channels[index]);
}

bool PatternGeneratorChannelGroup::isCollapsed() { return collapsed; }

void PatternGeneratorChannelGroup::collapse(bool val) { collapsed = val; }

void PatternGeneratorChannelGroup::append(
	PatternGeneratorChannelGroup *tojoin) {
...
////////////////////////// CHANNEL GROUP UI
PatternGeneratorChannelManagerUI *
PatternGeneratorChannelGroupUI::getManagerUi() const {
	return managerUi;
}
...
PatternGeneratorChannelGroupUI::PatternGeneratorChannelGroupUI(
	...
	PatternGeneratorChannelManagerUI *managerUi, QWidget *parent)
	: ChannelGroupUI(chg, parent), managerUi(managerUi), traceOffset(0),
	  traceHeight(0), topSep(nullptr), botSep(nullptr), chUiSep(nullptr) {
	this->chg = chg;
	checked = false;
	ui = new Ui::PGChannelGroup();
	setAcceptDrops(true);
...
PatternGeneratorChannelGroupUI::getChannelGroup() {
...
void PatternGeneratorChannelUI::split() {
	...
	auto chgIndex = chgIter - channelGroups->begin();
	auto channels = *(getManagerUi()
				  ->chm->get_channel_group(chgIndex)
				  ->get_channels());
	auto chIter = std::find(channels.begin(), channels.end(), ch);
...
int PatternGeneratorChannelGroupUI::getTraceOffset() { return traceOffset; }
...
void PatternGeneratorChannelGroupUI::enable(bool enabled) {
	...
	if (this->getChannelGroup() ==
	    managerUi->chm->getHighlightedChannelGroup()) {
		managerUi->pg->enableBufferUpdates(enabled);
	}
}

int PatternGeneratorChannelGroupUI::isChecked() { return checked; }

void PatternGeneratorChannelGroupUI::check(int val) { checked = val; }
...
PatternGeneratorChannelGroupUI::setupDecoder(const char *decstr,
	...
	for (auto j = 0; j < 2; j++) {
		for (; i < ch_count && reqCh; reqCh = reqCh->next, i++) {
			...
			chtracepair.second =
				getManagerUi()->main_win->view_->get_clone_of(
					index);
...
void PatternGeneratorChannelGroupUI::setupUARTDecoder() {
	...
	UARTPattern *uart =
		dynamic_cast<UARTPattern *>(getChannelGroup()->pattern);
	if (uart == nullptr)
		return;
	...
	if (getManagerUi()->getUseDecoders()) {
		...
		uartdecoder->set_option(
			"baudrate", g_variant_new_int64(uart->get_baud_rate()));
		uartdecoder->set_option("num_data_bits",
		...
		default:
			...
			uartdecoder->set_option("parity_check",
...
void PatternGeneratorChannelGroupUI::setupSPIDecoder() {
	...
	if (chg->get_channel_count() >= 2) {
		...
		if (getManagerUi()->getUseDecoders()) {
			...
			auto spipattern = dynamic_cast<SPIPattern *>(
				getChannelGroup()->pattern);
			...
			spidecoder->set_option(
				"cpol",
				g_variant_new_int64(spipattern->getCPOL()));
			spidecoder->set_option(
				"cpha",
				g_variant_new_int64(spipattern->getCPHA()));
			// spidecoder->set_option("wordsize",
			// g_variant_new_int64(spipattern->getBytesPerFrame()*8));

			auto decoderAnnotations = 3;

			for (auto i = 0; i < decoderAnnotations; i++) {
				ui->ann_row_layout->addWidget(
					new QLabel("", this));
			}
...
void PatternGeneratorChannelGroupUI::dropEvent(QDropEvent *event) {
	...
	if (formGroup &&
		...
		auto chgNOfChannels = getManagerUi()
					      ->chm->get_channel_group(chgIndex)
					      ->get_channel_count();
		getManagerUi()->chm->moveChannel(chgIndex, chgNOfChannels - 1,
						 0, false);
		getManagerUi()->chm->highlightChannel(
			getManagerUi()->chm->get_channel_group(chgIndex));
	} else {
...
PatternGeneratorChannelManager::PatternGeneratorChannelManager()
	...
	highlightedChannelGroup =
		static_cast<PatternGeneratorChannelGroup *>(channel_group[0]);
}

PatternGeneratorChannel *
PatternGeneratorChannelManager::get_channel(int index) {
...
void PatternGeneratorChannelManager::clearChannels() {
	for (auto ch : channel) {
		...
		ch->set_label(temp);
		PatternGeneratorChannel *pgch =
			static_cast<PatternGeneratorChannel *>(ch);
		if (pgch)
...
PatternGeneratorChannelGroup *
PatternGeneratorChannelManager::get_channel_group(int index) {
	return static_cast<PatternGeneratorChannelGroup *>(
		channel_group[index]);
}
...
void PatternGeneratorChannelManager::join(std::vector<int> index) {
	PatternGeneratorChannelGroup *src =
		static_cast<PatternGeneratorChannelGroup *>(
			channel_group[index[0]]);
	...
	for (auto i = 1; i < index.size(); i++) {
		...
		static_cast<PatternGeneratorChannelGroup *>(
			channel_group[index[0]])
			->append(static_cast<PatternGeneratorChannelGroup *>(
				channel_group[index[i]]));
		delete *it;
...
void PatternGeneratorChannelManager::split(int index) {
	...
	for (auto &&subch : *(channel_group[index]->get_channels())) {
		channel_group.insert(
			it,
			new PatternGeneratorChannelGroup(
				static_cast<PatternGeneratorChannel *>(subch),
				chgState));
		it++;
	}
...
void PatternGeneratorChannelManager::splitChannel(int chgIndex, int chIndex) {
	...
	// auto it = channel_group.begin()+chgIndex; // Use this to insert split
	// channel before channelgroup
	...
	// auto newChgIndex = chgIndex; // Use this to insert split channel
	// before channelgroup
...
PatternGeneratorChannel *
PatternGeneratorChannelManager::getHighlightedChannel() {
...
void PatternGeneratorChannelManager::generatePatterns(short *mainBuffer,
	...
	for (auto &&chg : channel_group) {
		...
		if (pgchg->is_enabled()) {
			pgchg->pattern->generate_pattern(
...
void PatternGeneratorChannelManager::commitBuffer(
	PatternGeneratorChannelGroup *chg, short *buffer, uint32_t bufferSize) {
	...
	for (auto i = 0; i < bufferSize; i++) {
		...
		buffer[i] = (buffer[i] & ~(chg->get_mask())) |
			remap_buffer(channel_mapping, val);
	}
}
...
uint32_t PatternGeneratorChannelManager::computeSuggestedSampleRate() {
	...
	for (auto &&chg : channel_group) {
		if (chg->is_enabled()) {
			auto patternSamplingFrequency =
				static_cast<PatternGeneratorChannelGroup *>(chg)
					->pattern->get_min_sampling_freq();

			if (patternSamplingFrequency == 0) {
				continue;
			}

			sampleRate = boost::math::lcm(patternSamplingFrequency,
						      sampleRate);
...
uint32_t PatternGeneratorChannelManager::computeSuggestedBufferSize(
	...
	for (auto &&chg : channel_group) {
		if (chg->is_enabled()) {
			auto patternBufferSize =
				static_cast<PatternGeneratorChannelGroup *>(chg)
					->pattern->get_required_nr_of_samples(
						sample_rate,
			...
			if (static_cast<PatternGeneratorChannelGroup *>(chg)
				    ->pattern->is_periodic()) {
				bufferSize = boost::math::lcm(patternBufferSize,
							      bufferSize);
				qDebug()
					<< static_cast<
						   PatternGeneratorChannelGroup
	...
	// if buffersize not big enough, create a buffer that is big enough and
	// will fit maximum nonperiodic pattern TBD if correct
	if (maxNonPeriodic > bufferSize) {
		auto result = 1;

		for (auto i = 1;
		     result < maxBufferSize && maxNonPeriodic > result; i++) {
			result = bufferSize * i;
		}
...
PatternGeneratorChannelManagerUI::PatternGeneratorChannelManagerUI(
	...
	PatternGenerator *pg)
	: QWidget(parent), ui(new Ui::PGChannelManager), cgSettings(cgSettings),
	  settingsWidget(cgSettings->patternSettings), main_win(main_win_),
	  m_use_decoders(true) {
	ui->setupUi(this);
	this->chm = chm;
	this->pg = pg;
...
void PatternGeneratorChannelManagerUI::updateUi() {
	static const int channelGroupLabelMaxLength =
		pg->channelGroupLabelMaxLength;
	static const int dioLabelMaxLength = 2;

	if (pg->getCurrentPatternUI())
		disconnect(pg->getCurrentPatternUI(), SIGNAL(decoderChanged()),
	...
	for (auto ch : chg_ui) {
		ch->hide();
		ch->setMouseTracking(
			false); // prevent hovering after the channel is deleted
		ch->deleteLater();
	}
	...
	Util::setWidgetNrOfChars(chmHeader->labelName,
	...
	for (auto &&ch : *(chm->get_channel_groups())) {
		...
		PatternGeneratorChannelGroupUI *currentChannelGroupUI =
			chg_ui.back();
		...
		ui->verticalLayout->insertWidget(
			ui->verticalLayout->count() - 1, chg_ui.back());
		currentChannelGroupUI->ensurePolished();
		currentChannelGroupUI->topSep = prevSep;

		Util::retainWidgetSizeWhenHidden(
			currentChannelGroupUI->ui->collapseBtn);
		Util::retainWidgetSizeWhenHidden(
			currentChannelGroupUI->ui->splitBtn);

		Util::setWidgetNrOfChars(
			currentChannelGroupUI->ui->ChannelGroupLabel,
			channelGroupLabelMaxLength);

		QString channelGroupLabel =
			QString().fromStdString(ch->get_label());

		if (channelGroupLabel.length() > channelGroupLabelMaxLength) {
			channelGroupLabel.truncate(channelGroupLabelMaxLength -
						   2);
			channelGroupLabel += "..";
		}

		currentChannelGroupUI->ui->ChannelGroupLabel->setText(
			channelGroupLabel);
		Util::setWidgetNrOfChars(currentChannelGroupUI->ui->DioLabel,
					 dioLabelMaxLength);

		connect(static_cast<PatternGeneratorChannelGroupUI *>(
				chg_ui.back()),
			SIGNAL(channel_enabled()), this,
			SIGNAL(channelsChanged())); // TEMP
		connect(currentChannelGroupUI->ui->selectBox,
			SIGNAL(toggled(bool)),
			static_cast<PatternGeneratorChannelGroupUI *>(
				chg_ui.back()),
			SLOT(select(bool)));
		...
		if (ch->is_grouped()) { // create subwidgets
			...
			connect(currentChannelGroupUI->ui->collapseBtn,
				SIGNAL(clicked()), chg_ui.back(),
				SLOT(collapse()));
			connect(currentChannelGroupUI->ui->splitBtn,
				SIGNAL(clicked()), chg_ui.back(),
				SLOT(split()));
			...
			if (!currentChannelGroupUI->getChannelGroup()
				     ->isCollapsed()) {
				currentChannelGroupUI->chUiSep = addSeparator(
					currentChannelGroupUI->ui
						->subChannelLayout,
					currentChannelGroupUI->ui
						->subChannelLayout->count());
				prevSep = currentChannelGroupUI->chUiSep;

				for (auto i = 0; i < ch->get_channel_count();
				     i++) {

					currentChannelGroupUI->ch_ui.push_back(
						new PatternGeneratorChannelUI(
							static_cast<
								PatternGeneratorChannel
									*>(
								ch->get_channel(
					...
					Util::setWidgetNrOfChars(
						...
						channelGroupLabelMaxLength);
					Util::setWidgetNrOfChars(
						currentChannelUI->ui->DioLabel,
						dioLabelMaxLength);

					currentChannelUI->ui
						->wgChannelEnableGroup
						->setVisible(false);
					currentChannelUI->ui->collapseBtn
						->setVisible(false);
					currentChannelUI->ui->selectBox
						->setVisible(false);

					connect(currentChannelUI->ui->splitBtn,
						SIGNAL(clicked()),
						currentChannelUI,
						SLOT(split()));
					connect(currentChannelUI,
						SIGNAL(requestUpdateUi()), this,
						SLOT(triggerUpdateUi()));

					auto str = QString().fromStdString(
						ch->get_channel(i)
							->get_label());
					currentChannelUI->ui->ChannelGroupLabel
						->setText(str);

					auto index =
						ch->get_channel(i)->get_id();
					str = QString().number(index);
					currentChannelUI->ui->DioLabel->setText(
						str);

					auto trace1 =
						main_win->view_->get_clone_of(
							index);
					currentChannelUI->setTrace(trace1);
				}

				// Remove last separator from subchannellayout
				// and add it to the main layout, so that
				// collapse channelgroup shows both separators
				// correctly
				currentChannelGroupUI->ui->subChannelLayout
					->removeWidget(prevSep);
				currentChannelGroupUI->botSep = addSeparator(
					ui->verticalLayout,
					ui->verticalLayout->count() - 1);
				prevSep = currentChannelGroupUI->botSep;
				currentChannelGroupUI->ch_ui.back()->botSep =
					prevSep;
				prevSep = currentChannelGroupUI->botSep;
			} else {
				currentChannelGroupUI->botSep = addSeparator(
				...
				currentChannelGroupUI->ui->collapseBtn
					->setChecked(true);
				currentChannelGroupUI->ui->subChannelWidget
					->setVisible(false);
			}
			...
			auto index = currentChannelGroupUI->getChannelGroup()
					     ->get_channel(0)
					     ->get_id();
			auto trace1 = main_win->view_->get_clone_of(index);
			currentChannelGroupUI->setTrace(trace1);
			currentChannelGroupUI->ui->DioLabel->setText(
				QString().number(ch->get_channel()->get_id()));
			currentChannelGroupUI->ui->splitBtn->setVisible(false);
			currentChannelGroupUI->ui->collapseBtn->setVisible(
				false);
			currentChannelGroupUI->botSep =
				addSeparator(ui->verticalLayout,
					     ui->verticalLayout->count() - 1);
			prevSep = currentChannelGroupUI->botSep;
			currentChannelGroupUI->chUiSep =
				currentChannelGroupUI->botSep;
		}

		// only enable channelgroup controls after all channelgroup
		// subchannels have been created
		currentChannelGroupUI->ui->enableBox->setChecked(
			ch->is_enabled());
		currentChannelGroupUI->enableControls(ch->is_enabled());
		connect(currentChannelGroupUI->ui->enableBox,
			SIGNAL(toggled(bool)), chg_ui.back(),
			SLOT(enable(bool)));
	}
	...
	ui->scrollArea->setMaximumWidth(
		channelManagerHeaderWiget->sizeHint().width());
...
void PatternGeneratorChannelManagerUI::triggerUpdateUi() { updateUi(); }
...
void PatternGeneratorChannelManagerUI::showDisabled() { disabledShown = true; }

void PatternGeneratorChannelManagerUI::hideDisabled() { disabledShown = false; }
...
void PatternGeneratorChannelManagerUI::showHighlight(bool val) {
	PatternGeneratorChannelGroupUI *uiChg =
		findUiByChannelGroup(chm->getHighlightedChannelGroup());
	PatternGeneratorChannelUI *uiCh =
		findUiByChannel(chm->getHighlightedChannel());
...
std::vector<PatternGeneratorChannelGroupUI *>
PatternGeneratorChannelManagerUI::getEnabledChannelGroups() {
