...
LogicAnalyzerChannel::LogicAnalyzerChannel(uint16_t id_,
	...
	: Channel(id_, label_), channel_role(nullptr), trigger("none"),
	  ch_thickness(1.0) {}
...
const srd_decoder *LogicAnalyzerChannelGroup::getDecoder() { return decoder; }
...
LogicAnalyzerChannelGroupUI::LogicAnalyzerChannelGroupUI(
	...
	QWidget *parent)
	: ChannelGroupUI(chg, parent), ui(new Ui::LAChannelGroup),
	  traceOffset(0), traceHeight(0), botSep(nullptr), topSep(nullptr),
	  chUiSep(nullptr) {
	this->ui->setupUi(this);
	this->lchg = chg;
	this->chm_ui = chm_ui;
...
void LogicAnalyzerChannelGroupUI::set_decoder(const std::string &value) {
	//	static_cast<LogicAnalyzerChannelGroup
	//*>(chg)->setDecoder(value);
...
LogicAnalyzerChannelManager::LogicAnalyzerChannelManager() : ChannelManager() {
	for (auto i = 0; i < 16; i++) {
...
LogicAnalyzerChannelManagerUI::LogicAnalyzerChannelManagerUI(
	...
	: QWidget(parent), ui(new Ui::LAChannelManager), hidden(false),
	  collapsed(false), settings_exclusive_group(new QButtonGroup(this)),
	  managerHeaderWidget(nullptr), locationSettingsWidget(settingsWidget),
	  settingsUI(nullptr), currentSettingsWidget(nullptr),
	  hoverWidget(nullptr), generalSettings(nullptr),
	  locationSettingsLayout(settingsLayout), streaming_mode(false) {
	ui->setupUi(this);
	main_win = main_win_;
	this->chm = chm;
	this->la = la;
...
QFrame *LogicAnalyzerChannelManagerUI::addSeparator(QVBoxLayout *lay, int pos) {
...
void LogicAnalyzerChannelManagerUI::deselect_all() {
	for (auto ch : chg_ui) {
		ch->ui->selectCheckBox->setChecked(false);
	}
}
	...
	if (chm->getHighlightedChannelGroup()) {
		...
		if (chGroup->is_grouped()) {
			...
			for (auto optch : chGroup->decoderOptChannels) {
				...
				/* Add all the available channels + "none" to
				 * the list */
				optChUI->roleCombo->addItem("-");
				for (auto &&ch : *(chGroup->get_channels())) {
					optChUI->roleCombo->addItem(
