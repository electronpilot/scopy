...
int drop_untill_tag_arives::general_work(int noutput_items,
	...
	for (int i = 0; i < ninput_items_.size(); ++i) {
		...
		//		// Get tags in range and print each tag key value
		//pairs + index in buffer 		std::vector<gr::tag_t> tags; 		uint64_t
		//nr = nitems_read(i); 		get_tags_in_range(tags, i, nr, nr +
		//noutput_items + 1);

		//		if (tags.size()) {
		//			std::cout << "No of tags: " << tags.size()
		//<< " value: " << tags[0].offset
		//				  << " Interval: " << nr << " - " <<
		//nr + noutput_items + 1 << std::endl; 		} else { 			std::cout << "
		//Interval: " << nr << " - " << nr + noutput_items + 1 <<
		//std::endl;
		//		}
